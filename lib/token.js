...
token.prototype = {
    init: (options) => {
        options = options || {}
        clientConnect.host = options.host || '127.0.0.1';
        clientConnect.port = options.port || '6379';
        if (options.password) {
            clientConnect.password = options.password || ''
        }
        client = redis.createClient(clientConnect);
    },
